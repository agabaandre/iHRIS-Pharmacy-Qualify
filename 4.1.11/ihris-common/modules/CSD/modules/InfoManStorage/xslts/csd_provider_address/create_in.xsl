  ...
  <xsl:template match="/">         
	...
	<xsl:for-each select="/csd:CSD/csd:providerDirectory/csd:provider">
	  <csd:provider>
	    ...
	    <xsl:for-each select="./csd:demographic/csd:address">
	      <csd:contactPoint><xsl:attribute name="id"><xsl:value-of select="../../@entityID"/>:<xsl:value-of select="@type"/></xsl:attribute></csd:contactPoint>
	    </xsl:for-each>
	  </csd:provider>
	</xsl:for-each>
      </csd:providerDirectory>
