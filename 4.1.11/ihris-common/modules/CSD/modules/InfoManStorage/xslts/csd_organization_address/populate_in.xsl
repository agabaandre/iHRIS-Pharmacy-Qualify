...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">         
    <xsl:for-each select="/csd:CSD/csd:organizationDirectory/csd:organization/csd:address[1]">      
      <form name='csd_organization_address' parent_form='csd_organization'>
	<xsl:attribute name="id"><xsl:value-of select="concat(../@entityID,'/',@type)"/></xsl:attribute>
