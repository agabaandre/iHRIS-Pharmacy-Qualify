...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">         
    <xsl:for-each select="/csd:CSD/csd:providerDirectory/csd:provider/csd:organizations/csd:organization/csd:contactPoint[1]">      
      <form name='csd_provider_organization_contact_point' parent_form='csd_provider_organization'>
	...
	<xsl:if test="./csd:codedType">        
	  <field name="csd_contact_point_type" type="MAP"><value form='csd_contact_point_type'><xsl:value-of select="concat(csd:codedType/@codingScheme,'@@@',csd:codedType/@code)"/></value></field> 
	</xsl:if>
      </form>
    </xsl:for-each>
  </xsl:template> 
