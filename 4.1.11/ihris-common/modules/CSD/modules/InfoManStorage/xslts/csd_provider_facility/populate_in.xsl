...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">         
    <xsl:for-each select="/csd:CSD/csd:providerDirectory/csd:provider/csd:facilities/csd:facility">        
      <form name='csd_provider_facility' parent_form='csd_provider'>
	...
	<field name='csd_facility' type='MAP'>
	  <value form='csd_facility'><xsl:value-of select="@entityID"/></value>
	</field>
      </form>
    </xsl:for-each>
  </xsl:template> 
