...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">     
  ...
  <xsl:template match="/">         
    <csd:careServicesRequest xmlns:csd="urn:ihe:iti:csd:2013" xmlns="urn:ihe:iti:csd:2013">             
      <function urn='urn:openhie.org:openinfoman-hwr:stored-function:health_worker_indices_service'>
	<requestParams>                     
	  <xsl:choose>
	    <xsl:when test="/form/@parent_id">
	      ...
	      <facility><xsl:attribute name='entityID'><xsl:value-of select="substring-after(/form/@parent_id,'/')"/></xsl:attribute></facility>
	    </xsl:when>
	    <xsl:otherwise>
	      <id/>
	      <facility/>
