...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">         
    <xsl:for-each select="/csd:CSD/csd:facilityDirectory/csd:facility/csd:otherID">      
      <form name='csd_facility_otherid' parent_form='csd_facility'>
	...
	<xsl:if test="@code">        
	  <field name="csd_facility_otherid_type" type="STRING_LINE"><xsl:value-of select="@code"/></field> 
	</xsl:if>
	<xsl:if test="text()">        
