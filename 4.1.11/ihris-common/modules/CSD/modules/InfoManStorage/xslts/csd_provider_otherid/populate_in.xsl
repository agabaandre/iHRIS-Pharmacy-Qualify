...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">         
    <xsl:for-each select="/csd:CSD/csd:providerDirectory/csd:provider/csd:otherID">      
      <form name='csd_provider_otherid' parent_form='csd_provider'>
	...
	<xsl:if test="@code">        
	  <field name="csd_otherid_type" type="STRING_LINE"><xsl:value-of select="@code"/></field> 
	</xsl:if>
	<xsl:if test="text()">        
