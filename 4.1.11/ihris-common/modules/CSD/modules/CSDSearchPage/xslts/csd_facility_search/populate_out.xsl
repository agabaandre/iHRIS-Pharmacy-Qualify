...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">     
  ...
  <xsl:template match="/">         
	...
	<requestParams>                      
	  ...
	  <xsl:if test="/form/field[@name='number'] and substring-after(/form/field[@name='csd_facility_otherid_type']/value,'@@@')">
	    <otherID>
	      <xsl:attribute name='code'><xsl:value-of select="substring-after(/form/field[@name='csd_facility_otherid_type']/value,'@@@')"/></xsl:attribute>
	  ...
	  <xsl:for-each select="/form/field[@name='csd_facility_type']/value[1]">
	    <codedType>
	  ...
	  <record>
	    <xsl:for-each select="/form/field[@name='csd_facility_status']/value[1]">
