...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">     
  ...
  <xsl:template match="/">         
	...
	<requestParams>                      
	  ...
	  <xsl:if test="/form/field[@name='number'] and substring-after(/form/field[@name='csd_otherid_type']/value,'@@@')">
	    <otherID>
	      <xsl:attribute name='code'><xsl:value-of select="substring-after(/form/field[@name='csd_otherid_type']/value,'@@@')"/></xsl:attribute>
	  ...
	  <xsl:if test="/form/field[@name='common_name']">
	    <commonName><xsl:value-of select="/form/field[@name='common_name']"/></commonName>
	  </xsl:if>	  
	  <xsl:for-each select="/form/field[@name='csd_provider_type']/value[1]">
	    <codedType>
	  ...
	  <organizations>
	    <xsl:for-each select="/form/field[@name='csd_organization']/value[1]">
	      <organization>
		<xsl:attribute name='entityID'><xsl:value-of select="/form/field[@name='csd_organization']/value[1]"/></xsl:attribute>
	      </organization>
	    </xsl:for-each>
	  </organizations>
	  <facilities>
	    <xsl:for-each select="/form/field[@name='csd_facility']/value[1]">
	      <facility>
		<xsl:attribute name='entityID'><xsl:value-of select="/form/field[@name='csd_facility']/value[1]"/></xsl:attribute>
	      </facility>
	    </xsl:for-each>
	  </facilities>
	  <xsl:for-each select="/form/field[@name='csd_language']/value[1]">
	    <language>
	      <xsl:attribute name='codingScheme'><xsl:value-of select="substring-before(text(),'@@@')"/></xsl:attribute>
	      <xsl:attribute name='code'><xsl:value-of select="substring-after(text(),'@@@')"/></xsl:attribute>
	    </language> 
	  ...
	  <record>
	    <xsl:for-each select="/form/field[@name='csd_provider_status']/value[1]">
