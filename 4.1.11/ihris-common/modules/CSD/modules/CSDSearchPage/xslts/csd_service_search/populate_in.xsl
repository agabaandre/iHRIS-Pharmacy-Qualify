...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <form name='csd_service_search' id='0'>
      <field name='result' type='STRING_MLINE'><xsl:apply-templates select="*" mode="XmlEscape" /></field>
      <field name='csd_service' type='MAP_MULT'>
	<xsl:for-each select="/csd:CSD/csd:serviceDirectory/csd:service">
	  <value form='csd_service'><xsl:value-of select="@entityID"/></value>
	</xsl:for-each>
      </field>
    </form>
  </xsl:template>         
