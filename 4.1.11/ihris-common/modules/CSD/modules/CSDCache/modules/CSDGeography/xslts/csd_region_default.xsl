...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="rootRegionURN"/>
    <xsl:param name="rootOrganizationURN"/>
    <xsl:param name="sourceDirectory"/>
    <xsl:param name="currentDateTime"/>

    <xsl:variable name="orgtype">REGION</xsl:variable> <!-- This should be replaces by a standardized list at some point, but what? -->
    ...
    <xsl:template match="/">
      <xsl:for-each
        ...
        <xsl:variable name="organizationURN">urn:uuid:<xsl:value-of select="normalize-space(form[@name='region']/field[@name='csd_uuid']/text())"/></xsl:variable>
        ...
        <xsl:variable name="name" select="normalize-space(form[@name='region']/field[@name='name']/text())"/>
        <xsl:variable name="code" select="normalize-space(form[@name='region']/field[@name='code']/text())"/>
        <xsl:variable name="hidden" select="normalize-space(form[@name='region']/field[@name='i2ce_hidden']/text())"/>
        <xsl:variable name="country" select="joinedForms/joinedForm[@report_form_name='country']/form[@name='country']/field[@name='csd_uuid']/text()"/>
        <xsl:variable name="type" select="$orgtype"/>
      ...
      <xsl:if test="$code != ''">
	    ...
	    <xsl:if test="$country != ''">
	      <csd:parent>
                <xsl:attribute name="entityID">urn:uuid:<xsl:value-of select="$country"/></xsl:attribute>
	      </csd:parent>
	    </xsl:if>
