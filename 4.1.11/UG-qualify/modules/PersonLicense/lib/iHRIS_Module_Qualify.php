...
class iHRIS_Module_Qualify extends I2CE_Module {
    ...
    public static function getHooks() {
        return array(
                'validate_form_person_license' => 'validate_form_person_license',
    ...
    public function action_deployment( $page ) {
        ...
        return $page->addLastChildForm( 'deployment', 'deployment_date' );
    }
    ...
    public function action_training( $page ) {
	    ...
	    //$obj->populateChildren( array( "registration"), array( "registration" => "registration_date") );
	    $obj->populateChildren(array( "registration", "person_provisional" ),array( "registration" => "registration_date", "person_provisional" => "issue_date" ) );
	    ...
	    if ( array_key_exists( "person_provisional", $obj->children ) ) {
                $page->getTemplate()->setForm( current( $obj->children["person_provisional"] ), $node );
            } else {
                $page->getTemplate()->setForm( $factory->createContainer( "person_provisional" ), $node );
            }
        }
        return true;
    }
     ...
     */
    public function validate_form_person_license( $form ) {
    ...
    public function validate_form_training_disrupt( $form ) {
        if ( I2CE_Validate::checkDate( $form->disruption_date ) && I2CE_Validate::checkDate( $form->resumption_date ) ) {
            if ( $form->resumption_date->before( $form->disruption_date ) ) {
                $form->getField( "resumption_date" )->setInvalid( "The resumption date must be after the disruption date." );
            }
        }
    }
    ...
    public function validate_form_exam( $form ) {
        if ( I2CE_Validate::checkDate( $form->exam_date ) && I2CE_Validate::checkDate( $form->application_date ) ) {
            if ( $form->exam_date->before( $form->application_date ) ) {
                $form->getField( "exam_date" )->setInvalid("The exam date must be after the spplication date." );
            }
        }       
    }
    ...
    public function validate_form_training( $form ) {
        if ( !I2CE_Validate::checkMap( $form->training_program ) && !I2CE_Validate::checkMap( $form->out_qualification ) ) {
            $form->getField( "training_program" )->setInvalid("You must select a Qualification from Trained in Country or Trained outside the Country." );
        } elseif ( I2CE_Validate::checkMap( $form->out_cadre ) ) {
            if ( !I2CE_Validate::checkMap( $form->out_country ) ) {
                $form->getField( "out_country" )->setInvalid( "You must select the Country where training was received." );                            
            }
            if ( !I2CE_Validate::checkDate( $form->graduation ) ) {
                $form->getField( "graduation" )->setInvalid("This field is required when you selected training outside the country." );
            }
        }
        if ( I2CE_Validate::checkDate( $form->intake_date ) && I2CE_Validate::checkDate( $form->graduation ) ) {
            if ( $form->graduation->before( $form->intake_date ) ) {
                $form->getField( "graduation" )->setInvalid( "The graduation date must be after the intake date." );
            }
        }
    }
    ...
    public function validate_form_disciplinary_action( $form ) {
        if ( I2CE_Validate::checkDate( $form->reinstate_date ) && I2CE_Validate::checkDate( $form->action_date ) ) {
            if ( $form->reinstate_date->before( $form->action_date ) ) {
                $form->getField( "reinstate_date" )->setInvalid( "The reinstatement date must be after the initial action date." );
            }
        }
    }
    ...
    public function upgrade($old_vers,$new_vers) {
        ...
        if ( I2CE_Validate::checkVersion( $old_vers, "<", "3.3.3" ) ) {
            ...
            unset( $class_config->iHRIS_QualifyPerson->fields->home_country );
            unset( $class_config->iHRIS_QualifyPerson->fields->home_district );
            unset( $class_config->iHRIS_QualifyPerson->fields->home_county );
        }
        if ( I2CE_Validate::checkVersion( $old_vers, "<", "3.3.5" ) ) {
            $user = new I2CE_User( 1, false, false, false );
            $class_config = I2CE::getConfig()->modules->forms->formClasses;
