 ...
 * @access public
 */
class iHRIS_PageFormPersonLicense extends iHRIS_PageFormParentPerson {
    ...
    protected function loadObjects() {
        ...
        if ( !$this->isPost() && !$this->getPrimary()->license_number ) {
            ...
            $parent->populateChildren( "training" );
	    foreach( $parent->children["training"] as $train ) {
		$train->populateChildren( "registration");
                if ( array_key_exists( "registration", $train->children ) ) {
                  $reg = current( $train->children['registration'] );
