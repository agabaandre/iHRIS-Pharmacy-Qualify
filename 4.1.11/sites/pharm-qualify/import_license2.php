...
define( "iHRIS_LICENSE_SERIAL" , 6 ); 
define( "iHRIS_LICENSE_RECIEPT", 7 );
define( "iHRIS_LICENSE_START_DATE" , 4 ); 
define( "iHRIS_LICENSE_END_DATE", 5 );
...
while ( ( $data = fgetcsv( $fh ) ) !== false ) {
	...
	//Creating/issuing a license to a person who didn't have one previously
		...
		$license->setParent( $person->getNameId() );
			...
			if ( $data[iHRIS_LICENSE_SERIAL] ) {
				$license->serial_number = $data[iHRIS_LICENSE_SERIAL];
			}
			if ( $data[iHRIS_LICENSE_RECIEPT] ) {
				$license->receipt_number = $data[iHRIS_LICENSE_RECIEPT];
			}
			if ( $data[iHRIS_LICENSE_START_DATE] ) {
