...
define( 'iHRIS_INSTITUTION', 0 );
//define( 'iHRIS_AGENT', 1 );
//define( 'iHRIS_LOCATION', 2 );
...
//print_r ($cache['training_program']);
//print_r ($cache['training_institution']);
$row = 0;
$errors = 0;
while ( ( $data = fgetcsv( $fh ) ) !== false ) {
    ...
    // The following will create the qualification if it doesn't exist...
    //if ( !find_or_create( $data[iHRIS_QUALIFICATION], "qualification", false, true ) ) {
    ...
    // The following will create the cadre if it doesn't exist...
    //if ( $data[iHRIS_CADRE] && !find_or_create( $data[iHRIS_CADRE], "cadre", false, true ) ) {
...
while ( ( $data = fgetcsv( $fh ) ) !== false ) {
	...
	if( $data[iHRIS_INSTITUTION] != "" ) 
	{
        if ( array_key_exists( $data[iHRIS_QUALIFICATION] . ' - ' . $data[iHRIS_INSTITUTION], $cache['training_program'] ) ) {
            $exists++;
            continue;
        }
        $institution_id = find_or_create( $data[iHRIS_INSTITUTION], "training_institution", false, true );


        $qualification_id = find_or_create( $data[iHRIS_QUALIFICATION], "qualification" );
        $cadre_id = find_or_create( $data[iHRIS_CADRE], "cadre" );

        $training_program_id = find_or_create( $data[iHRIS_QUALIFICATION] . ' - ' . $data[iHRIS_INSTITUTION],
                "training_program", array(
                    "training_institution" => $institution_id,
...
fclose($fh);
echo "\n$exists already existed in dB\n Created $no_institution Institution  \n Skipped $Skip_no_institution because of no institution name.\n";
