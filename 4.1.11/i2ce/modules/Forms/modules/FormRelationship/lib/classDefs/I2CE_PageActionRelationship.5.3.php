...
class I2CE_PageActionRelationship extends I2CE_Page {
    ...
    protected function sortIterator($data) {
        $sort_functions = array();
        if (array_key_exists('sort_functions',$this->args) && is_array($this->args['sort_functions'])) {
            foreach ($this->args['sort_functions'] as $index=>$sdata) {
                if (!is_array($sdata)  
                    || !array_key_exists('field',$sdata)
                    || !$sdata['field']
                    || !array_key_exists('compare',$sdata)
                    || !is_callable($sdata['compare'])
                    ) {
                    continue;
                }
                list($form,$field) = array_pad(explode('+',$sdata['field'],2),2,'');
                if (!$form || !$field) {
                    continue;
                }
                $sort_functions[$index] = array(
                    'compare'=>$sdata['compare'],
                    'form'=>$form,
                    'field'=>$field);
            }
        }
        if ( count($sort_functions) == 0) {
            //we don't need to sort
            return  $data;
        }
        ksort($sort_functions);
        //we need to sort
        $t_data = array();
        foreach ($data as $id=>$formfields) {
            $t_data[$id] = $formfields;
        }
        array_reverse($sort_functions,true);
        foreach ($sort_functions as $sdata) {
            $form=$sdata['form'];
            $field=$sdata['field'];
            $callable = $sdata['compare'];
            $func = function ($a,$b) use ($form,$field,$callable){
                if (!is_array($a) 
                    || ! array_key_exists($form,$a)
                    || ! is_array($a[$form])   
                    || ! array_key_exists($field,$a[$form])
                    ) {
                    return -1;
                }
                if (!is_array($b) 
                    || ! array_key_exists($form,$b)
                    || ! is_array($b[$form])   
                    || ! array_key_exists($field,$b[$form])
                    ) {
                    return 1;
                }         
                return call_user_func($callable,$a[$form][$field],$b[$form][$field]);
            };
            uasort($t_data,$func);    
        }
        return $t_data;
