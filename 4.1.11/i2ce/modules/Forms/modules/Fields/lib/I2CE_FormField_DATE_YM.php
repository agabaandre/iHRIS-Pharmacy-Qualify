 ...
 * @access public
 */
class I2CE_FormField_DATE_YM extends I2CE_FormField_DB_DATE { 
    public function loadFromXML($node) {
        if (!$node instanceof DOMElement) {
            return;
        }        
        $expected_values = array('month','year');
        $values = array();
        ...
        $this->setValue(I2CE_Date::getYearMonth(  $values['month'], $values['year'] ));        
    }
    ...
    public function setFromPost( $post ) {
        if (is_array($post)) {
            if ( array_key_exists('month', $post ) ) {
                $this->setValue( I2CE_Date::getYearMonth( $post['month'], $post['year'] ));
                return;
            }
        ...
        $this->setValue( I2CE_Date::getYearMonth());
    }
    ...
    public function processDOMNotEditable($node,$template,$form_node) {
        ...
        I2CE_Date::addYearMonthElement( $template, $ele_name, $this->getValue(), false, $node, false, true );
   ...
   /**
    ...
    public function getFromDB( $value ) {
        ...
        $value->setType(I2CE_Date::YEAR_MONTH);
        return $value;
    }          
    ...
    public function setValue( $value ) {
        ...
        $this->value->setType(I2CE_Date::YEAR_MONTH);
    }
