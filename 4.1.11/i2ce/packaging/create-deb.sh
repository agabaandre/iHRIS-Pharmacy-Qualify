...
echo Current tagged version is $LASTVERS.
...
do
    ...
    echo  "echo Building Package $PKG  on Release $RLS for Target $TARGET.  Major Release $MJRRLS"
    ...
    if [ "$PKG" != "i2ce" ]; then
	$BZR checkout --lightweight $PARENT  $SRCDIR/i2ce
    fi
    ...
    $PHP $SRCDIR/i2ce/tools/translate_templates.php && echo $?
